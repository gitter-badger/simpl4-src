...
public class BaseProducer extends DefaultProducer {
	...
	public BaseProducer(BaseEndpoint endpoint, long timeout, long timeResolution) {
		super(endpoint);
	...
	protected void copyResultToCamel(Exchange exchange, ProcessInstance pi) {
		...
		Map<String, Object> returnVars = getBaseEndpoint().getReturnVarMap();
	...
	protected void signal(Exchange exchange) {
		...
		while (firstTime || (timeout > 0 && (System.currentTimeMillis() - initialTime < timeout))) {
			try {
				...
				throw new RuntimeException("error occured while waiting for activity=" + activity + " for processInstanceId=" + processInstanceId);
			}
			firstTime = false;
		...
		runtimeService.setVariables(execution.getId(), ExchangeUtils.prepareVariables(exchange, getBaseEndpoint()));
	...
	protected ProcessInstance startProcess(Exchange exchange) {
		BaseEndpoint endpoint = getBaseEndpoint();
	...
	protected BaseEndpoint getBaseEndpoint() {
		return (BaseEndpoint) getEndpoint();
	}
}
