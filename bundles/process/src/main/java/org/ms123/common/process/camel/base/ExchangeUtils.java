...
/**
 * This class contains one method - prepareVariables - that is used to copy variables from Camel into Process.
 * 
...
public class ExchangeUtils {
	...
	/**
	 * Copies variables from Camel into Process.
	 * 
	 * This method will copy the Camel body to the "camelBody" variable. It will copy the Camel body to individual variables within Process if it is of type 
	 ...
	 * <li>If the copyVariablesFromHeader parameter is set on the endpoint, each Camel Header will be copied to an individual variable within Process.</li>
	 * </ul>
	 * 
	 * @param exchange The Camel Exchange object
	 * @param baseEndpoint The BaseEndpoint implementation
	 * @return A Map&lt;String, Object&gt; containing all of the variables to be used in Process
	 */
	...
	public static Map<String, Object> prepareVariables(Exchange exchange, BaseEndpoint baseEndpoint) {
		Map<String, Object> camelVarMap = new HashMap<String, Object>();

		String copyProperties = baseEndpoint.getCopyVariablesFromProperties();
		...
		String copyHeader = baseEndpoint.getCopyVariablesFromHeader();
		if (!StringUtils.isEmpty(copyHeader) && (!isBoolean(copyHeader) || Boolean.parseBoolean(copyHeader))) {
			...
			boolean isSetProcessInitiator = baseEndpoint.isSetProcessInitiator();
			for (Map.Entry<String, Object> header : exchange.getIn().getHeaders().entrySet()) {
				// Don't pass the process initiator header as a variable.

				if ((!isSetProcessInitiator || baseEndpoint.getProcessInitiatorHeaderName().equals(header.getKey())) && (pattern == null || pattern.matcher(header.getKey()).matches())) {
		...
		if (camelBody instanceof Map<?, ?>) {
			...
			if (baseEndpoint.isCopyCamelBodyToBodyAsString() && !(camelBody instanceof String)) {
	 ...
	 */
	public static String prepareInitiator(Exchange exchange, BaseEndpoint baseEndpoint) {

		String initiator = null;
		if (baseEndpoint.isSetProcessInitiator()) {
			try {
				initiator = exchange.getIn().getHeader(baseEndpoint.getProcessInitiatorHeaderName(), String.class);
			} catch (TypeConversionException e) {
				throw new RuntimeException("Initiator header '" + baseEndpoint.getProcessInitiatorHeaderName() + "': Value must be of type String.", e);
			}

			if (StringUtils.isEmpty(initiator)) {
				throw new RuntimeException("Initiator header '" + baseEndpoint.getProcessInitiatorHeaderName() + "': Value must be provided");
			}
		}
		return initiator;
	}
