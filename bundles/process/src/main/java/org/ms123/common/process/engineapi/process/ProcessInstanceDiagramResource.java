...
public class ProcessInstanceDiagramResource extends BaseResource {
	...
	public String getDiagram() {
		...
		if (pde != null && pde.isGraphicalNotationDefined()) {
			//@@@MS BpmnModel bpmnModel = getPE().getRepositoryService().getBpmnModel(pde.getId());
			//@@@MS DefaultProcessDiagramGenerator dp = new DefaultProcessDiagramGenerator();
			//@@@MS resource = dp.generateDiagram(bpmnModel, "png", getPE().getRuntimeService().getActiveActivityIds(m_processInstanceId),new java.util.ArrayList(),null,null,DefaultProcessDiagramGenerator.class.getClassLoader(),1.0d);
		} else {
