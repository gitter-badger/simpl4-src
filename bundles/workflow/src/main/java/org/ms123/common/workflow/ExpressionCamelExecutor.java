...
public class ExpressionCamelExecutor {
	...
	public static Object execute(Map<String, Object> methodDefinition, VariableScope scope) {
		try {
			...
			activitiProperties.put(HISTORY_ACTIVITI_ACTIVITY_KEY, pi.getTenantId() + "/" + getProcessName(pi.getProcessDefinitionId()) + "/" + pi.getId() + "/" + pi.getActivityId());
			activitiProperties.put(WORKFLOW_ACTIVITY_ID, pi.getActivityId());
			activitiProperties.put(WORKFLOW_ACTIVITY_NAME, pi.getName());
			...
			activitiProperties.put(WORKFLOW_PROCESS_DEFINITION_NAME, pi.getProcessDefinitionName());
