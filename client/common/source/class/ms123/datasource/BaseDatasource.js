...
qx.Class.define("ms123.datasource.BaseDatasource", {
	...
	members: {
      ...
      var datasource = { 'osgi.jdbc.driver.name':data.name,
      ...
      var jooq = { 'create_jooq_metadata':data.create_jooq_metadata,
										'jooq_inputschema':data.jooq_inputschema,
										'jooq_includes':data.jooq_includes,
										'jooq_excludes':data.jooq_excludes
										};
      var datanucleus = { 'create_datanucleus_metadata':data.create_datanucleus_metadata,
				...
				this.fireDataEvent("save", JSON.stringify(config,null,2), null);
			}
		},
		_getDbList:function(){
			...
			for( var i =0; i < databases.length; i++){
				dbList.push( { value: databases[i].name, label: databases[i].name} );
			}
			return dbList;
		},
